...
export async function saveProfileToBlockchain(
	data,
	setStatusMessage = () => {},
	saveOnChain = undefined
) {
	try {
		setStatusMessage("Uploading profile data to Pinata...");
		const { ipfsUrl } = await uploadToIPFS(data, setStatusMessage);
		console.log("IPFS URL:", ipfsUrl);

		if (typeof window === "undefined" || !window.ethereum) {
			throw new Error(
				"Ethereum provider not found. Please install MetaMask and run this code in the browser."
			);
		}
		await window.ethereum.request({ method: "eth_requestAccounts" });
		const provider = new BrowserProvider(window.ethereum);
		const signer = await provider.getSigner(); // Get the transaction-capable signer.
		const signerAddress = await signer.getAddress();
		console.log("Signer address:", signerAddress);

		const contract = new Contract(CONTRACT_ADDRESS, CONTRACT_ABI, signer);
		setStatusMessage("Submitting transaction to register profile on-chain...");
		const tx = await contract.registerUser(ipfsUrl);
		await tx.wait();
		setStatusMessage("Profile registered on blockchain successfully.");
		console.log("Transaction:", tx);

		if (saveOnChain) {
			saveOnChain({ tx, ipfsUrl });
		}
		return { tx, ipfsUrl };
	} catch (error) {
		setStatusMessage(`Error saving profile on blockchain: ${error.message}`);
		console.error("Error saving profile on blockchain:", error);
		throw error;
	}
}
